...
def wait_for_new_task_or_alert(interval_seconds=120, stop_event=None):
    """
    Called when pending folder is empty.
    Alerts user every `interval_seconds`, but checks every second if a new file appeared in pending.
    Returns True if a new task was detected, False if stopped.
    """
    print(f"\n[QUEUE EMPTY] Pending folder is empty! Will alert via Telegram every {interval_seconds}s until new task appears.")
    
    # Send initial alert
    send_telegram_message("📢 *Comrade Yoan!* All tasks in `pending` folder are completed!\nWaiting for new tasks...")
    ...
    while True:
        ...
        # Check if new files dropped into pending
        pending_files = list(config.PENDING_DIR.glob("*.txt"))
        if pending_files:
            print(f"[QUEUE] Detected {len(pending_files)} new task(s) in pending! Resuming work...")
            send_telegram_message(f"🚀 *New task detected:* `{pending_files[0].name}`. Resuming automation!")
            return True
        ...
        if elapsed >= interval_seconds:
            send_telegram_message("⏳ *Reminder:* Pending folder is still empty. Machine is resting in the bunker.")
            elapsed = 0
